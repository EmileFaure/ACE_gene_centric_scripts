...
#>50% R2
Gene_Mat=readRDS("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/GM_AGN_T60MAXGQ_transposed_NZVuniquecut20_ATT_RSquared50.rds")
...
ggplot() +   geom_hline(yintercept = 0, linetype='dotted') +
  ...
  labs(col = "Water mass")
ggsave("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/01-FIGURES/RDA_Triplot_Attached_R2supp15.png",width=29.7,height=21,unit="cm")
...
RF=select(RF,c("AGC_ID","R2_caret"))
agcscore=merge(agcscore,RF,by.x="row.names",by.y="AGC_ID")
ggplot() +   geom_hline(yintercept = 0, linetype='dotted') +
  geom_vline(xintercept = 0, linetype='dotted') +
  labs(x = paste0("RDA1 (",
                      round(100 * scores$Eigenval.rel[1], 1), "%)"),
           y = paste0("RDA2 (",
                      round(100 * scores$Eigenval.rel[2], 1), "%)")) +
  theme(plot.title=element_text(hjust=0.5)) +
  geom_point(aes(x = agcscore$RDA1, y = agcscore$RDA2, col=agcscore$R2_caret),size = 0.5, alpha = 0.4)
ggsave("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/01-FIGURES/RDA_AGCplot_Attached_R2supp50_colbyR2.png",width=29.7,height=21,unit="cm")
...
saveRDS(res.rda,"/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/RDA/RDA_Object_PostVariableSelection_Attached_R2supp15.rds")
