...
ggsave("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/01-FIGURES/Density_R2_FreeLiving_T60MAX.pdf")

# Adding results from threshold sum30

#RF_S30=fread("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/Threshold_Sum_30_Objects/RF_AGC_T30_NZVuniquecut20_FL_All.txt",data.table=F)
#ggplot() +
#  geom_density(aes(x=  RF$R2_caret), fill="deepskyblue3", col = "deepskyblue4", alpha = 0.6) +
#  geom_density(aes(x=  RF_S30$R2_caret), fill="firebrick", col = "firebrick", alpha = 0.6) +
#  geom_segment(aes(x=mean(RF$R2_caret,na.rm=T), xend=mean(RF$R2_caret,na.rm=T), y=0, yend=2.6), linetype = "dashed", color = "darkblue") +
#  geom_segment(aes(x=mean(RF_S30$R2_caret,na.rm=T), xend=mean(RF_S30$R2_caret,na.rm=T), y=0, yend=2.6), linetype = "dashed", color = "firebrick4") +
#  labs(y = "Density", x = expression(R^{2} ~ "of cross-validated random forest models")) +
#  theme(axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 14), axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16))

#ggsave("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/01-FIGURES/Density_R2_FreeLiving_T60MAX_AND_S30.pdf")
...
bpall
ggsave("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/01-FIGURES/Importance_Boxplot_FreeLiving_T60MAX.png",width=29.7,height=21,unit="cm")

# Permutations
RF_Perm_1994 = fread("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/RF_AGC_TMAX60GQ_NZVuniquecut20_FL_Permuted1994_All.txt",data.table=F)
RF_Perm_1997 = fread("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/RF_AGC_TMAX60GQ_NZVuniquecut20_FL_Permuted1997_All.txt",data.table=F)

ggplot() +
  geom_density(aes(x=  RF$R2_caret), fill="deepskyblue3", col = "deepskyblue4", alpha = 0.6) +
  geom_segment(aes(x=mean(RF$R2_caret,na.rm=T), xend=mean(RF$R2_caret,na.rm=T), y=0, yend=30), linetype = "dashed", col = "deepskyblue4") +
  geom_density(aes(x=  RF_Perm_1994$R2_caret), fill="indianred1", col = "indianred2", alpha = 0.6) +  
  geom_segment(aes(x=quantile(RF_Perm_1994$R2_caret,na.rm=T,0.95), xend=quantile(RF_Perm_1994$R2_caret,na.rm=T,0.95), y=0, yend=30), linetype = "dashed", col = "indianred2") +
  geom_density(aes(x=  RF_Perm_1997$R2_caret), fill="indianred3", col = "indianred4", alpha = 0.6) +  
  geom_segment(aes(x=quantile(RF_Perm_1997$R2_caret,na.rm=T,0.95), xend=quantile(RF_Perm_1997$R2_caret,na.rm=T,0.95), y=0, yend=30), linetype = "dashed", col = "indianred4") +
  labs(y = "Density", x = expression(R^{2} ~ "of cross-validated random forest models")) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 14), axis.text.y = element_text(size = 14), axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16))

ggsave("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/01-FIGURES/Density_R2_T60MAX_FreeLiving_WithPerm_95percentile.pdf")
...
ggsave("/home/datawork-lmee-intranet-nos/ACE/06-STATS-GENE-MAT/01-FIGURES/Density_R2_FreeLiving_ByAGCCat_T60MAX.pdf")
